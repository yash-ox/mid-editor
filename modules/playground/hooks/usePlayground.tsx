...
export const usePlayground = (id: string): UsePlaygroundReturn => {
  ...
  const loadPlayground = useCallback(async () => {
    ...
    try {
      ...
      const rawContent = safeData?.templateFiles[0]?.content;

      if (typeof rawContent === "string") {
        const parsedContent = JSON.parse(rawContent);
        setTemplateData(parsedContent);

        toast.success("Playground fetched successfully.");
        return;
      }
      ...
      if (
        templateRes.templateJson === Array.isArray(templateRes.templateJson)
      ) {
        setTemplateData({
          folderName: "Root",
